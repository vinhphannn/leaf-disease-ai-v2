"""
One-click evaluator with in-file config.

Usage: Just open this file and run it in your IDE (no CLI args needed).
"""
import os
import time
...
# ===== In-file config (edit here) =====
MODEL_PATH = "models/model_best.pth"
DATA_DIR = "./data/valid"  # change to ./data/test if needed
IMG_SIZE = 224
BATCH_SIZE = 32
NUM_WORKERS = 4
OUTPUTS_DIR = "./outputs"
...
def main():
    ...
    try:
        ...
        with open(os.path.join(OUTPUTS_DIR, 'runner_metrics.txt'), 'w', encoding='utf-8') as f:
        ...
        with open(os.path.join(OUTPUTS_DIR, 'runner_classification_report.txt'), 'w', encoding='utf-8') as f:
            f.write(report)

        import csv
        cm = confusion_matrix(labels, preds)
        with open(os.path.join(OUTPUTS_DIR, 'runner_confusion_matrix.csv'), 'w', newline='', encoding='utf-8') as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow([''] + classes)
            for i, row in enumerate(cm):
                writer.writerow([classes[i]] + row.tolist())
        print('Saved outputs under outputs/: runner_metrics.txt, runner_classification_report.txt, runner_confusion_matrix.csv')
    except Exception:
